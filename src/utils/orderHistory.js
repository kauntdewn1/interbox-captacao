...
export const saveOrderToHistory = (orderData) => {
    try {
        ...
        const isDuplicate = existingHistory.some(order => {
            ...
            return (order.produtoId === newOrder.produtoId &&
 ...
 */
export const removeOrderFromHistory = (produtoId, cor, tamanho, data) => {
    try {
        const history = getOrderHistory();
        const filteredHistory = history.filter(order => !(order.produtoId === produtoId &&
            order.cor === cor &&
 ...
 */
export const exportOrderHistory = () => {
    const history = getOrderHistory();
 ...
 */
export const importOrderHistory = (jsonData) => {
    try {
        ...
        // Validar estrutura
        const isValid = imported.every((order) => {
            const o = order;
            return (o &&
                typeof o.produtoId === 'string' &&
                typeof o.slug === 'string' &&
